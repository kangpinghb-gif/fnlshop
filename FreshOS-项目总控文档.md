...
当前版本已经推进到 `v1.1`，核心口径统一为：

- 每天 12:00 生成次日订货建议。
- 使用 0-12 点实时销售做轻量趋势修正。
- 使用预计到货前库存参与订货计算。
- Hermes 负责定时导出 / 抓取大表哥数据并触发 `freshos-worker`。
- 企业微信 / 飞书负责查看摘要、附件和异常。
...
- 大表哥日数据导入。
- 订单导入与门店 / 商品匹配。
- 库存口径、库存年龄、销量预测、订货建议、库存风险。
- 报表导出和摘要推送。
- 阿里云 ECS 部署文档和 systemd 模板。
...
4. 再用历史 0-12 点销售做轻量趋势修正。
5. 历史 0-12 点占比过低时，直接回退基础预测。
6. 仍无数据时，预测为 0，并写风险提示。
...
```text
建议订货量 = 预测销量 + 安全库存 + 已订未到修正 - 预计到货前库存
```
...
还没有接入真实生产 PostgreSQL，因此以下能力虽然已有代码，但未做正式生产验证：
...
还没有正式大表哥 `40/42` 导出数据，因此以下能力尚未用正式生产数据验证：
